...
class AppHandler(BaseHTTPRequestHandler):
    server_version = "CoConection/3.0"
    ...
    def do_POST(self):
        try:
            if self.path == "/api/open-url":
                ...
                self.api_save_ips()
            elif self.path == "/api/save-custom-config":
                self.api_save_custom_config()
            else:
    ...
    def api_save_custom_config(self):
        """Save a custom config to the output file."""
        data = self._read_json()
        config = (data.get("config") or "").strip()
        if not config:
            raise ValueError("Config is empty.")
        custom_file = OUT_DIR / "custom_configs.txt"
        existing = ""
        if custom_file.exists():
            existing = custom_file.read_text(encoding="utf-8", errors="ignore")
        custom_file.write_text((existing.rstrip() + "\n" + config + "\n"), encoding="utf-8")
        # Also add to quick import
        quick_file = OUT_DIR / "nova_quick_import.txt"
        if quick_file.exists():
            quick_content = quick_file.read_text(encoding="utf-8", errors="ignore")
            quick_file.write_text(config + "\n" + quick_content, encoding="utf-8")
        self._send_json({"ok": True, "saved": str(custom_file)})
